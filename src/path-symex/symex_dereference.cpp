...
exprt symex_dereferencet::read_object(
  const exprt &object,
  const exprt &offset,
  const typet &type)
{
...
exprt symex_dereferencet::dereference_rec(
  const exprt &address,
  const exprt &offset,
  const typet &type)
{
  ...
  {
    // failure
    return nullary_exprt("dereference_failure", type);
  }
}

exprt symex_dereferencet::dereference_if(
  const if_exprt &expr,
  const exprt &offset,
  const typet &type)
{
...
exprt symex_dereferencet::dereference_plus(
  const exprt &expr,
  const exprt &offset,
  const typet &type)
{
...
exprt symex_dereferencet::dereference_typecast(
